(()=>{
  ...
  const svg={
    ...
    edit:'<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4 7h16M7 4v6M4 17h16M17 14v6"/><path d="M4 12h16M12 9v6"/></svg>',
    back:'<svg viewBox="0 0 24 24" aria-hidden="true"><path d="m15 5-7 7 7 7"/></svg>'
  };
  const labels={home:'Home',shoot:'Shoot',review:'Review',learn:'Learn',edit:'Edit'};
  const routeTab={home:'home',shoot:'shoot',conditions:'shoot',review:'review',learn:'learn',camera:'learn',simulator:'learn',visuals:'learn',practice:'learn',edit:'edit'};
  const routeTitle={home:'Canon T7 Studio',shoot:'Guided Shoot',conditions:'Photo Conditions',review:'Photo Review',learn:'Learn',camera:'Camera Controls',simulator:'Exposure Simulator',visuals:'Visual Guides',practice:'Practice',edit:'Editor'};

  function rebuildDock(){
    const dock=$('.mobile-dock');if(!dock)return;
    dock.innerHTML=['home','shoot','review','learn','edit'].map(k=>`<a href="#${k}" class="${k==='shoot'?'shoot-dock':''}" aria-label="${labels[k]}">${svg[k]}<span>${labels[k]}</span></a>`).join('');
  }
  ...
  /* Consistent pressed feedback without changing feature logic. */
  ...
  function syncWeatherLoading(){
    if(!weatherStatus||!conditions)return;const t=weatherStatus.textContent||'';const loading=/loading|finding|requesting/i.test(t);
    conditions.classList.toggle('is-loading',loading);conditions.setAttribute('aria-busy',loading?'true':'false');
    if(search)search.disabled=loading;if(locate)locate.disabled=loading;
  }
  ...
  /* Review processing feedback. */
  const review=$('.review-flow'),file=$('#fileInput'),score=$('#reviewScore');
  let reviewLoading=null;
  if(review){
    reviewLoading=document.createElement('div');reviewLoading.className='polish-processing';reviewLoading.innerHTML='<span class="polish-spinner"></span><span>Checking exposure, detail and Canon settings…</span>';
    const host=review.querySelector('.review-v2-overview')||review;host.prepend(reviewLoading);
  }
  ...
  /* Make upgraded expandable details easier to understand/access. */
  $$('.rv2-details summary,.cv2-weather-details summary,.ev2-advanced summary').forEach(s=>s.setAttribute('role','button'));
})();
