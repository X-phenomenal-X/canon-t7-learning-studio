(()=>{
  ...
  const shoot=$('#shoot');
  function capturePulse(){const screen=shoot?.querySelector('.shoot-screen[data-screen="4"]');const ready=screen?.querySelector('.capture-ready-v2');if(!screen?.classList.contains('active')||!ready)return;ready.classList.remove('motion-capture-ready');requestAnimationFrame(()=>ready.classList.add('motion-capture-ready'));setTimeout(()=>ready.classList.remove('motion-capture-ready'),1100)}
  if(shoot)new MutationObserver(capturePulse).observe(shoot,{attributes:true,subtree:true,attributeFilter:['class']});
  window.addEventListener('t7-route-changed',e=>{if(e.detail?.route==='shoot')setTimeout(capturePulse,100)});

  const review=$('.review-flow');
  function reviewArrive(){const result=review?.querySelector('.rv2-result');if(!result||result.hidden)return;result.classList.remove('motion-photo-arrive');requestAnimationFrame(()=>result.classList.add('motion-photo-arrive'));setTimeout(()=>result.classList.remove('motion-photo-arrive'),700)}
  if(review)new MutationObserver(()=>{const result=review.querySelector('.rv2-result');if(result&&!result.hidden)reviewArrive()}).observe(review,{childList:true,subtree:true,attributes:true,attributeFilter:['hidden','class']});

  window.T7Motion={refresh(){registerReveal();updateRanges();capturePulse()}};
})();
