(()=>{
  ...
  simplifyNavigation();
  style('./shoot-flow.css');style('./learn.css');style('./review-flow.css');
  script('./t7-engine.js')
    .then(()=>script('./core.js'))
    .then(()=>script('./shoot-flow.js'))
    .then(()=>script('./learn.js'))
    .then(()=>script('./review-flow.js'))
    .then(()=>{const review=document.querySelector('.review-flow');if(review)review.id='review';simplifyNavigation()})
    .catch(showLoadError);
