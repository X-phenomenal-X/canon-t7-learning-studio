(()=>{
  ...
  section.innerHTML=`
    ...
    <div class="library-main">
      <div class="library-card"><div class="library-card-head"><div><small>TECHNICAL TREND</small><h2>Recent progress</h2><p>Focus/detail and exposure across your latest reviews.</p></div><select class="library-filter" id="libFilter" aria-label="Filter photos by subject"><option value="all">All photos</option><option value="portrait">Portraits</option><option value="product">Products</option><option value="landscape">Landscapes</option><option value="action">Action</option><option value="indoor">Indoor</option><option value="night">Night</option><option value="general">General</option></select></div><div class="library-chart" id="libChart"></div><div class="library-chart-legend"><span class="focus">Focus / detail</span><span class="exposure">Exposure</span></div></div>
  ...
  function chart(list){
    ...
    const grid=[75,50].filter(g=>g>lo).map(g=>`<line x1="${p}" y1="${y(g)}" x2="${W-p}" y2="${y(g)}" stroke="#26313d"/><text x="${p}" y="${y(g)-6}" fill="#7c8b9c" font-size="10">${g}</text>`).join('');
